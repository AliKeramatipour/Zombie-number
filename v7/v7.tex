...
zombie player must move each zombie to one of its adjacent vertices so that they get closer to the survivor, while the
survivor player either moves to an adjacent vertex or stays at his current location. Here lies the difference
between {\it Zombies and Survivor} and {\it Cops and Robber(s)} games, as in {\it Cops and Robber(s)} cops should not
necessarily get closer to the robber(s), they can either hold their current position, get closer, or further away from
the robber(s). Although zombies are not as intelligent as cops, they can still choose their path intelligently between
the shortest paths. If any zombie and the survivor ever occupy the same vertex, the survivor is captured and the zombie
player wins. The zombie number of a graph $G$, denoted as $\zn(G)$, is the minimum number of zombies required so that
the zombie player can always capture the survivor, no matter how survivor moves.
...
{\it Capture time} in a game with $z_c$ zombies for the zombie player, is the maximum number of rounds, that survivor
can survive. If this goes to infinity, a survivor-win play exists.
...
\begin{proof}
	...
	Suppose not, this path uses some {\it G-edge}s and some {\it H-edge}s. If a {\it G-edge} is followed by an {\it
	H-edge} (or vice-versa), then we can swap these edges and still end up in the same vertex. For example if $(u_1,v_1)
	\rightarrow (u_2,v_1) \rightarrow (u_2,v_2)$ is happening, we can do $(u_1,v_1) \rightarrow (u_1,v_2) \rightarrow
	(u_2,v_2)$. Since we can swap each two edges of different type, suppose all {\it G-edge}s happen before all {\it
	H-edge}s in the shortest path. Since this path has a length less than $dist_G(x,u) + dist_H(y,v)$, we have found a
...
\section{Limited capture time zombie number problem is NP-Hard}\label{np-capturetime}
	...
	Also $N_G[u]$ represents the closed neighborhood of $u$ in graph $G$.
	...
	\begin{proof}
		...
		$S$, then there is no vertex $v$ such that $v \in S'$ and $v \in N_{G'_k}[u] \cup \{(u,1 \leq i < k)\}$. By having
		the survivor in vertex $(u, k-1)$, there is no zombie at distance $k$ or less from him, which means he will not
	...
	\begin{proof}
		...
		to vertices in $N_G[j]$ (See figure \ref{fig:p8}). Since we are adding $2n^2$ new vertices, building $H$
		...
		means after their move all of them should be at one of $(k,v,b)$'s neighbors, that is, $N_G[v] $ or, $ (k,1
