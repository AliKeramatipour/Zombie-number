...
we prove that the original {\it Zombies and Survivor} game belongs to {\it NP-Hard} class of problems.
...
\begin{proof}
	...
	Suppose using this strategy the survivor wins, therefore survivor must be able to do either infinite {\it G-move}s
	or infinite {\it H-move}s. Without loss of generality, suppose the survivor makes infinite {\it H-move}s. After
	$len(p_G)$ number of {\it H-move}s, {\it H-zombie}s will get to $H_x$. Now for each {\it G-move} made by the
...
\end{proof}
An example for further understanding can be found at \cite{CartesianProductExample}.
...
\begin{thebibliography}{999}
	...
	W. B. Kinnersley, ``Cops and Robbers is EXPTIME-complete", Journal of Combina- torial Theory Series B, 111 (2015), 201–220.
	\bibitem{CartesianProductExample}
	``An example for zombie number of Cartesian product of two graphs'', 2020 available at: link.link.link
\end{thebibliography}
