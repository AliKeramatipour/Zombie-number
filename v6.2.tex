...
\section{Limited capture time zombie number Problem is NP-Hard}\label{np-capturetime}
	...
	Define $Lcz(G,k)$ (limited capture time zombie number) as the minimum number of zombies needed so that zombie player is able to capture the survivor in
	...
	\begin{proof}
		...
		be captrued. Thus $\gamma(G) \leq Lcz(G',k)$ holds.
		...
		be captured in at most $k$ moves. Therefore, $Lcz(G',k) \leq \gamma(G)$.

		By combining results, $Lcz(G',k) = gamma(G)$. Therefore dominating-set problem is reduced to $Lcz_k$.
	...
	the problem $Lcz_{(n + 1) * n^2}$, and by solving $Lcz_{(n + 1) * n^2}$ for $G$, we get $z(G)$ as well.
	...
	\begin{proof}
		...
		It is now proved that $z(H) = |MDS(G)|$, thus the dominating-set problem is reduced to $NPZ$.
