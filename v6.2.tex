...
\begin{document}
	...
	\begin{frontmatter}
		...
		survivor, no matter what moves survivors make. In this paper, we prove a conjecture by Fitzpatrick et
...
\cite{Fitz16} is played as follows (note that we only consider the game with a single survivor or robber in this paper).
Initially, the zombie player chooses a number $z$ and places $z$ zombies on the graph vertices. Then the survivor player
chooses one single vertex which is the survivor's initial position. Starting with the zombie player, on each player's
turn, survivor player either moves to an adjacent vertex or stays at his current location, while zombie player must move
each zombie to one of its adjacent vertices so that they get closer to the survivor. Here lies the difference between
{\it Zombies and Survivor} and {\it Cops and Robber} games, as in {\it Cops and Robber} cops should not necessarily
get closer to the {\it Robber}, they can either hold their current position, get closer, or further away from the
robber. If any zombie and the survivor ever occupy the same vertex, the survivor is captured and the zombie player wins.
The zombie number of a graph $G$, denoted as $z(G)$, is the minimum number of zombies required so that the zombie player
can always capture the survivor no matter how survivor moves.
...
A $G$-equivalent graph, is defined as a graph where each zombie in an arbitrary set of them, say in vertex $(z_x,z_y)$, is
...
\begin{proof}
	...
	$z(G)$ zombies, that have a winning strategy on a single $G$, say $G_{z = 1}$ and call them {\it G-zombies}. We do the
	same for $H_{s = 1}$ and call them {\it H-zombies}.


	Consider one of the shortest paths between vertices $z$(the $G$-subgraph shared by {\it G-zombies}) and $y$ in $H$
	...
	$G$-subgraph. Now consider when $z = y$, {\it G-zombies} will play their winning strategy (that they had on a single
	$G$) in this case. This move is also possible since in $G$'s strategy, zombies would get closer to survivor on each
	turn. If $z = y$ and survivor makes an {\it H-move}, {\it G-zombies} will maintain their positioning by mimicking
	the exact same move on their corresponding $H$. This means for those turns that $z=y$ holds, if we consider the
	$G$-equivalent graph between {\it G-zombies} and survivor, it is just like a simple game played on a single $G$.
	{\it H-zombies} will follow the same strategy but in their corresponding environment.
	...
	we prove that this is not possible. After $len(p_G)$ number of {\it H-move}s, {\it H-zombies} will get to $H_x$. Now for
...
\section{Limited capture time zombie number Problem is NP-Hard}\label{np-capturetime}
	...
	We define limited capture time zombie number ($LCZ_k$) problem as below:
	...
	\end{theorem}
	\begin{proof}
		We prove this by reducing a well-known NP-Hard problem (dominating set problem) to $LCZ_k$ in polynomial time.
		...
		By combining results, $z = |minimumDominatingSet(G)|$. Therefore dominating set problem is reduced to $LCZ_k$.
