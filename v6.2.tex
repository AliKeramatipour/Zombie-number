...
{\it Zombies and Survivor} and {\it Cops and Robbers} games, as in {\it Cops and Robbers} cops should not necessarily
...
\section{Capture time and Cartesian product}\label{capturetime}
	...
	$CT(G,z)$ is the maximum number of turns that survivor can avoid being caught, assuming that both players choose their best
...
\section{Limited capture time zombie number Problem is NP-Hard}\label{np-capturetime}
	...
	QUESTION: Is $LCZ(G,k) \leq z$ ? In other words, can we capture the survivor after at most $k$ turns using $z$ zombies in graph $G$ ?
